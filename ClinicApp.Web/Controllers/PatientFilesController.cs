...
public class PatientFilesController : Controller
{
    ...
    // GET /patients/{patientId}/files/{fileId}/xray-viewer
    [HttpGet("{fileId}/xray-viewer")]
    public async Task<IActionResult> XrayViewer(int patientId, int fileId, CancellationToken ct)
    {
        var file = await GetFileAsync(patientId, fileId, ct);
        if (file == null) return NotFound();
        ...
        ViewData["Title"] = $"X-ray Viewer — {file.OriginalFileName}";
        return View(vm);
    }
