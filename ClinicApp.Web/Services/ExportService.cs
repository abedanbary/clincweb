...
public byte[] ExportPatientPaymentReport(Patient patient, List<Treatment> treatments, List<Payment> payments, string clinicName)
{
    using var package = new ExcelPackage();
    var ws = package.Workbook.Worksheets.Add("Payment Report");

    var green = Color.FromArgb(16, 185, 129);
    var headerBg = Color.FromArgb(15, 23, 42);

    // Title block
    ws.Cells["A1:G1"].Merge = true;
    ws.Cells["A1"].Value = clinicName;
    ws.Cells["A1"].Style.Font.Size = 18; ws.Cells["A1"].Style.Font.Bold = true;
    ws.Cells["A1"].Style.Font.Color.SetColor(green);
    ws.Cells["A1"].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

    ws.Cells["A2:G2"].Merge = true;
    ws.Cells["A2"].Value = $"Patient Payment Report — {patient.FirstName} {patient.LastName}";
    ws.Cells["A2"].Style.Font.Size = 13; ws.Cells["A2"].Style.Font.Bold = true;
    ws.Cells["A2"].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

    ws.Cells["A3:G3"].Merge = true;
    ws.Cells["A3"].Value = $"Generated: {DateTime.Now:MMMM dd, yyyy 'at' HH:mm}";
    ...
    {
        var txPayments = payments.Where(p => p.TreatmentId == treatment.Id).OrderBy(p => p.PaymentDate).ToList();
        var totalPaid = txPayments.Where(p => p.Status == PaymentStatus.Paid).Sum(p => p.Amount);
        var remaining = Math.Max(0, treatment.Cost - totalPaid);

        // Treatment header row
        ws.Cells[row, 1, row, 7].Merge = true;
        ws.Cells[row, 1].Value = $"  {treatment.Title}  ({treatment.Type})";
        ws.Cells[row, 1].Style.Font.Bold = true;
        ws.Cells[row, 1].Style.Font.Color.SetColor(Color.White);
        ws.Cells[row, 1].Style.Fill.PatternType = ExcelFillStyle.Solid;
        ws.Cells[row, 1].Style.Fill.BackgroundColor.SetColor(headerBg);
        row++;

        // Summary row
        ws.Cells[row, 1].Value = "Total Cost"; ws.Cells[row, 2].Value = treatment.Cost;
        ws.Cells[row, 3].Value = "Total Paid"; ws.Cells[row, 4].Value = totalPaid;
        ws.Cells[row, 5].Value = "Remaining";  ws.Cells[row, 6].Value = remaining;
        ...
        ws.Cells[row, 4].Style.Font.Color.SetColor(green);
        ws.Cells[row, 6].Style.Font.Color.SetColor(remaining > 0 ? Color.OrangeRed : green);
        for (int c = 1; c <= 7; c++)
        {
        ...
        {
            // Column headers
            var cols = new[] { "Date", "Amount", "Method", "Status", "Notes" };
            for (int i = 0; i < cols.Length; i++)
            {
                ...
                ws.Cells[row, i + 1].Style.Fill.BackgroundColor.SetColor(green);
                ws.Cells[row, i + 1].Style.Font.Color.SetColor(Color.White);
            }
            row++;

            foreach (var p in txPayments)
            {
                ws.Cells[row, 1].Value = p.PaymentDate.ToLocalTime().ToString("MMM dd, yyyy");
                ws.Cells[row, 2].Value = p.Amount;
                ws.Cells[row, 2].Style.Numberformat.Format = "#,##0.00";
                ws.Cells[row, 3].Value = p.Method.ToString();
                ws.Cells[row, 4].Value = p.Status.ToString();
                ws.Cells[row, 4].Style.Font.Color.SetColor(
                    p.Status == PaymentStatus.Paid ? green :
                    p.Status == PaymentStatus.Refunded ? Color.Gray : Color.OrangeRed);
                ws.Cells[row, 5].Value = p.Notes ?? "—";
                if (row % 2 == 0)
                    for (int c = 1; c <= 5; c++)
                    {
                        ws.Cells[row, c].Style.Fill.PatternType = ExcelFillStyle.Solid;
        ...
        else
        {
            ...
            ws.Cells[row, 1].Value = "No payment records for this treatment.";
    ...
    // Grand totals
    var grandPaid = payments.Where(p => p.Status == PaymentStatus.Paid).Sum(p => p.Amount);
    var grandPending = payments.Where(p => p.Status == PaymentStatus.Pending).Sum(p => p.Amount);
    ws.Cells[row, 1, row, 7].Merge = true;
    ws.Cells[row, 1].Value = $"Grand Total Paid: {grandPaid:N2}     Grand Total Pending: {grandPending:N2}";
    ws.Cells[row, 1].Style.Font.Bold = true;
    ws.Cells[row, 1].Style.Font.Size = 12;
    ws.Cells[row, 1].Style.Font.Color.SetColor(headerBg);
...
public byte[] ExportOutstandingBalances(List<ViewModels.PatientBalanceSummary> balances, string clinicName)
{
    using var package = new ExcelPackage();
    var ws = package.Workbook.Worksheets.Add("Outstanding Balances");
    var green = Color.FromArgb(16, 185, 129);
    ...
    ws.Cells["A2"].Value = "Outstanding Patient Balances";
    ws.Cells["A2"].Style.Font.Size = 13; ws.Cells["A2"].Style.Font.Bold = true;
    ws.Cells["A2"].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
    ...
    int row = 5;
    var headers = new[] { "Patient Name", "Total Paid", "Amount Owed", "Pending Invoices", "Balance Status" };
    for (int i = 0; i < headers.Length; i++)
    {
    ...
    foreach (var b in balances.OrderByDescending(b => b.TotalPending))
    {
        ws.Cells[row, 1].Value = b.PatientName;
        ws.Cells[row, 2].Value = b.TotalPaid; ws.Cells[row, 2].Style.Numberformat.Format = "#,##0.00";
        ws.Cells[row, 3].Value = b.TotalPending; ws.Cells[row, 3].Style.Numberformat.Format = "#,##0.00";
        ws.Cells[row, 3].Style.Font.Color.SetColor(Color.OrangeRed);
        ws.Cells[row, 4].Value = b.PendingCount;
        ws.Cells[row, 5].Value = b.TotalPending > 0 ? "Outstanding" : "Cleared";
        ws.Cells[row, 5].Style.Font.Color.SetColor(b.TotalPending > 0 ? Color.OrangeRed : green);
        if (row % 2 == 0)
            for (int c = 1; c <= 5; c++)
            {
                ws.Cells[row, c].Style.Fill.PatternType = ExcelFillStyle.Solid;
    ...
    row++;
    ws.Cells[row, 1].Value = $"Total Outstanding: {balances.Sum(b => b.TotalPending):N2}";
    ws.Cells[row, 1].Style.Font.Bold = true;
    ws.Cells[row, 1].Style.Font.Color.SetColor(Color.OrangeRed);
